...
import { Direction, Color,Gender } from '@/types/enumclass.ts'
import { Person,type Persons} from '@/types/abstractClass.ts'
let person = ref({
  name: 'Person',
  age: 18
})
let car1 = ref({brand:' honda',color:Color.Blue,price:100000,location:Direction.Down})
...
function changePerson() {
  ...
  let personList:Persons = [{ name: 'Person2', age: 20, gender: Gender.Male, height: 180 }, { name: 'Person3', age: 20, gender: Gender.Male, height: 180 }]
}
function changeName(){
  person.value.name += '~'
}
